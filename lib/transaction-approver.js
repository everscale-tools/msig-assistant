...
function buildTransactionsIterationTask(client, iterator, txConfirmationQueue, txIterationStateDb) {
    return cb => {
        Async.waterfall([
            ...
            (iterationResult, cb) => {
                debug(`net.iterator_next returned with: ${JSON.stringify(iterationResult, null, 2)}`);

                iterationResult.items = _.filter(iterationResult.items, {
                    in_message: { msg_type: 1 },
                    out_messages: [{ msg_type: 2 }]
                });

                cb(null, iterationResult);
            },
            (iterationResult, cb) => {
                Async.each(iterationResult.items, (item, cb) => {
                    Async.parallel([
                        cb => {
                ...
                }, err => cb(err, iterationResult));
            },
            (iterationResult, cb) => {
                const txs = _
                    .chain(iterationResult.items)
            ...
            (iterationResult, cb) => {
                txIterationStateDb.update(
                    { key: 'resume-state' },
                    { $set: { value: iterationResult.state } },
                    { upsert: true },
                    cb
                );
            }
...
class TransactionApprover {
    constructor(config) {
        ...
        debug(`Creating TonClient with the following configuration: ${JSON.stringify(config.client, null, 2)}`);
        ...
        return new Promise((resolve, reject) => {
            Async.waterfall([
                ...
                (iterator, cb) => {
                    ...
                    Async.forever(Async.retryable(retryOpts, task), cb);
                }
            ], err => {
