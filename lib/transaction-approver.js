...
function buildTxConfirmationFn(dryRun) {
    if (dryRun) {
        return () => Promise.resolve();
    }

    return (client, transactionId, address, keys) => {
...
function initTxConfirmationQueue(everosConfig, dryRun, sources, destinations) {
    ...
    const txConfirmationQueue = Async.queue((txToConfirm, cb) => {
        ...
        const confirmTransaction = buildTxConfirmationFn(dryRun);
