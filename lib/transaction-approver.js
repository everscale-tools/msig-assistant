...
function initCursor(client, txIterationStateDb, aa) {
    return cb => {
        ...
        return client.net.query(params)
            .then(result => {
                const cursor = _.get(result, 'result.data.blockchain.account.transactions.pageInfo.endCursor');

                _.set(txIterationStateDb, ['data', 'cursors', aa, 'startCursor'], cursor);
...
function getEdges(client, txIterationStateDb, aa) {
    return (cursor, cb) => {
        ...
        client.net.query(params)
            .then(result => {
                const edges = _.get(result, 'result.data.blockchain.account.transactions.edges');
                ...
                _.set(txIterationStateDb, ['data', 'cursors', aa],
                    _.get(result, 'result.data.blockchain.account.transactions.pageInfo'));
...
function updateCursor(txIterationStateDb, aa) {
    return cb => {
        const cursor = _.get(txIterationStateDb, ['data', 'cursors', aa, 'endCursor']);
